...
namespace Aspose.Cells_FOSS.Samples.Validations
{
    internal static class Program
    {
        private static void Main()
        {
            ...
            var a1Validation = RequireValidation(loadedSheet.Validations.GetValidationInCell(0, 0), "A1");
            var c2Validation = RequireValidation(loadedSheet.Validations.GetValidationInCell(1, 2), "C2");
            var g1Validation = RequireValidation(loadedSheet.Validations.GetValidationInCell(0, 6), "G1");
            ...
            Console.WriteLine("G1 validation: " + g1Validation.Type);
        }

        private static Validation RequireValidation(Validation validation, string cellName)
        {
            if (validation == null)
            {
                throw new InvalidOperationException("Expected validation at " + cellName + ".");
            }
