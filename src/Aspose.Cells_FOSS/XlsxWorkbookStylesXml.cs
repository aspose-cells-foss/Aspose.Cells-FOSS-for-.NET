...
internal static class XlsxWorkbookStylesXml
{
    ...
    private static FontValue ReadFontValue(XElement font)
    {
        return new FontValue
        {
            ...
            Bold = font.Element(MainNs + "b") is not null,
            Italic = font.Element(MainNs + "i") is not null,
            Underline = font.Element(MainNs + "u") is not null,
            StrikeThrough = font.Element(MainNs + "strike") is not null,
    ...
    private static ColorValue ReadColorValue(XElement? colorElement)
    {
        ...
        if (!byte.TryParse(rgbValue.Substring(0, 2), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out var a))
        {
            return default;
        }
        if (!byte.TryParse(rgbValue.Substring(2, 2), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out var r))
        {
            return default;
        }
        if (!byte.TryParse(rgbValue.Substring(4, 2), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out var g))
        {
            return default;
        }
        if (!byte.TryParse(rgbValue.Substring(6, 2), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out var b))
        {
            return default;
        }
