    ...
    internal static class XlsxWorkbookArchiveHelpers
    {
        ...
        internal static Dictionary<string, string> LoadRelationships(ZipArchive archive, string relationshipsUri, string sourceUri)
        {
            ...
            foreach (var relationship in document.Root != null ? document.Root.Elements(XlsxWorkbookSerializerCommon.PackageRelationshipNs + "Relationship") : Enumerable.Empty<XElement>())
            {
                ...
                relationships[id] = ResolvePartUri(sourceUri, target);
            }
        ...
        internal static HashSet<int> LoadDateStyleIndexes(ZipArchive archive, IReadOnlyDictionary<string, string> workbookRelationships, LoadOptions options, LoadDiagnostics diagnostics)
        {
            ...
            for (var index = 0; index < cellXfs.Count; index++)
            {
                ...
                string formatCode;
                if (customFormats.TryGetValue(numFmtId.Value, out formatCode) && LooksLikeDateFormat(formatCode))
                {
                    dateStyleIndexes.Add(index);
                }
            }
        ...
        internal static int? ParseIntAttribute(XAttribute attribute)
        {
            ...
            int value;
            return int.TryParse(attribute.Value, NumberStyles.Integer, CultureInfo.InvariantCulture, out value) ? (int?)value : null;
        }
        ...
        internal static double? ParseDoubleAttribute(XAttribute attribute)
        {
            ...
            double value;
            return double.TryParse(attribute.Value, NumberStyles.Float, CultureInfo.InvariantCulture, out value) ? (double?)value : null;
        }
        ...
        {
            string worksheetUri;
            if (!string.IsNullOrWhiteSpace(relationshipId) && workbookRelationships.TryGetValue(relationshipId, out worksheetUri))
            {
                return worksheetUri;
            }

            var fallbackUri = "/xl/worksheets/sheet" + (sheetIndex + 1) + ".xml";
            if (GetEntry(archive, fallbackUri) != null)
            {
