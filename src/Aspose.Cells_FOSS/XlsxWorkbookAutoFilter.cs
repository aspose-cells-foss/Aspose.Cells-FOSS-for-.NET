...
internal static class XlsxWorkbookAutoFilter
{
    internal static XElement? BuildAutoFilterElement(WorksheetModel worksheet, int differentialFormatCount)
    {
        ...
        for (var index = 0; index < orderedColumns.Count; index++)
        {
            ...
            if (filterColumnElement is not null)
            {
    ...
    internal static void LoadAutoFilter(WorksheetModel worksheetModel, XElement worksheetRoot, XlsxWorkbookStyles.StylesheetLoadContext stylesheet, LoadDiagnostics diagnostics, LoadOptions options, string sheetName)
    {
        ...
        if (autoFilterElement is null)
        {
            return;
        }

        var reference = (string?)autoFilterElement.Attribute("ref");
        if (!AutoFilterSupport.TryNormalizeRange(reference, out var normalizedRange))
        {
        ...
        if (sortStateElement is not null)
        {
    ...
    {
        if (TryParseBooleanAttribute(filterColumnElement.Attribute("hiddenButton"), false, out var hiddenButton))
        {
            return hiddenButton;
        }

        if (filterColumnElement.Attribute("hiddenButton") is not null)
        {
            return WarnAndReturnFalse(diagnostics, options, sheetName, autoFilterRange);
        }

        if (TryParseBooleanAttribute(filterColumnElement.Attribute("showButton"), true, out var showButton))
        {
            return !showButton;
        }

        if (filterColumnElement.Attribute("showButton") is not null)
        {
            return WarnAndReturnFalse(diagnostics, options, sheetName, autoFilterRange);
        }
    ...
    private static void LoadCustomFilters(FilterColumnModel model, XElement filterColumnElement, LoadDiagnostics diagnostics, LoadOptions options, string sheetName, string autoFilterRange)
    {
        ...
        if (customFiltersElement is null)
        {
            return;
        }

        if (TryParseBooleanAttribute(customFiltersElement.Attribute("and"), false, out var andValue))
        {
            model.CustomFiltersAnd = andValue;
        }
        else if (customFiltersElement.Attribute("and") is not null)
        {
            if (options.StrictMode)
            {
        ...
        foreach (var customFilterElement in customFiltersElement.Elements(MainNs + "customFilter"))
        {
            ...
            if (!AutoFilterSupport.TryParseOperator((string?)customFilterElement.Attribute("operator"), out var operatorType))
            {
                if (options.StrictMode)
                {
    ...
    private static void LoadColorFilter(AutoFilterColorFilterModel model, XElement? colorFilterElement, XlsxWorkbookStyles.StylesheetLoadContext stylesheet, LoadDiagnostics diagnostics, LoadOptions options, string sheetName, string autoFilterRange)
    {
        ...
        if (colorFilterElement.Attribute("dxfId") is not null)
        {
        ...
        if (TryParseBooleanAttribute(colorFilterElement.Attribute("cellColor"), false, out var cellColor))
        {
            model.CellColor = cellColor;
        }
        else if (colorFilterElement.Attribute("cellColor") is not null)
        {
            if (options.StrictMode)
            {
    ...
    {
        model.Clear();
        if (dynamicFilterElement is null)
        {
            return;
        }
    ...
    {
        model.Clear();
        if (top10Element is null)
        {
            return;
        }

        model.Enabled = true;
        if (TryParseBooleanAttribute(top10Element.Attribute("top"), true, out var topValue))
        {
            model.Top = topValue;
        }
        else if (top10Element.Attribute("top") is not null)
        {
            if (options.StrictMode)
            {
        ...
        if (TryParseBooleanAttribute(top10Element.Attribute("percent"), false, out var percentValue))
        {
            model.Percent = percentValue;
        }
        else if (top10Element.Attribute("percent") is not null)
        {
            if (options.StrictMode)
            {
    ...
    private static void LoadSortState(AutoFilterSortStateModel model, XElement sortStateElement, XlsxWorkbookStyles.StylesheetLoadContext stylesheet, LoadDiagnostics diagnostics, LoadOptions options, string sheetName, string autoFilterRange)
    {
        ...
        if (TryParseBooleanAttribute(sortStateElement.Attribute("columnSort"), false, out var columnSort))
        {
            model.ColumnSort = columnSort;
        }
        else if (sortStateElement.Attribute("columnSort") is not null)
        {
            WarnSortState(diagnostics, options, sheetName, autoFilterRange);
        }

        if (TryParseBooleanAttribute(sortStateElement.Attribute("caseSensitive"), false, out var caseSensitive))
        {
            model.CaseSensitive = caseSensitive;
        }
        else if (sortStateElement.Attribute("caseSensitive") is not null)
        {
            WarnSortState(diagnostics, options, sheetName, autoFilterRange);
        }
        ...
        foreach (var sortConditionElement in sortStateElement.Elements(MainNs + "sortCondition"))
        {
            ...
            if (!AutoFilterSupport.TryNormalizeRange(conditionReference, out var normalizedConditionRef))
            {
                if (options.StrictMode)
                {
            ...
            if (TryParseBooleanAttribute(sortConditionElement.Attribute("descending"), false, out var descending))
            {
                condition.Descending = descending;
            }
            else if (sortConditionElement.Attribute("descending") is not null)
            {
                WarnSortState(diagnostics, options, sheetName, autoFilterRange);
            }

            var sortBy = NormalizeSortBy((string?)sortConditionElement.Attribute("sortBy"));
            if (sortConditionElement.Attribute("sortBy") is not null && string.IsNullOrEmpty(sortBy))
            {
                WarnSortState(diagnostics, options, sheetName, autoFilterRange);
            }
            ...
            if (sortConditionElement.Attribute("dxfId") is not null)
            {
            ...
            if (sortConditionElement.Attribute("iconId") is not null)
            {
    ...
    private static double? ParseOptionalDouble(XAttribute? attribute, LoadDiagnostics diagnostics, LoadOptions options, string sheetName, string autoFilterRange, string attributeLabel)
    {
        ...
        if (double.TryParse(attribute.Value, NumberStyles.Float, CultureInfo.InvariantCulture, out var parsed))
        {
            return parsed;
        }
