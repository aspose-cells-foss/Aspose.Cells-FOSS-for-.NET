...
internal static class XlsxWorkbookSerializer
{
    ...
    public static void Save(WorkbookModel model, Stream stream, SaveOptions options)
    {
        ...
        foreach (var worksheet in model.Worksheets)
        {
            foreach (var pair in worksheet.Cells)
            {
                ...
                if (pair.Value.Kind == CellValueKind.String && pair.Value.Value is string text && options.UseSharedStrings)
                {
        ...
        WriteXmlEntry(archive, "[Content_Types].xml", BuildContentTypes(model, sharedStrings.Values.Count > 0, stylesheet.HasStyles, corePropertiesDocument is not null, extendedPropertiesDocument is not null));
        WriteXmlEntry(archive, "_rels/.rels", BuildRootRelationships(corePropertiesDocument is not null, extendedPropertiesDocument is not null));
        ...
        for (var i = 0; i < model.Worksheets.Count; i++)
        {
            ...
            if (worksheetHyperlinkRelationships is not null)
            {
        ...
        if (corePropertiesDocument is not null)
        {
            WriteXmlEntry(archive, "docProps/core.xml", corePropertiesDocument);
        }

        if (extendedPropertiesDocument is not null)
        {
    ...
    public static WorkbookModel Load(Stream stream, LoadOptions options, LoadDiagnostics diagnostics)
    {
        ...
        try
        {
            ...
            for (var index = 0; index < sheetElements.Count; index++)
            {
                ...
                worksheetDefinedNames.TryGetValue(index, out var definedNamesState);
        ...
        WorksheetDefinedNamesState? definedNamesState)
    {
        ...
        {
            if (!TryResolveRowIndex(rowElement, diagnostics, options, sheetName, out var rowIndex))
            {
                continue;
            }
            ...
            {
                if (!TryReadCellRecord(cellElement, rowIndex, dateSystem, sharedStrings, stylesheet, diagnostics, options, sheetName, out var address, out var record))
                {
                    continue;
                }
        ...
        if (worksheetRoot.Element(MainNs + "dimension") is null && (worksheetModel.Cells.Count > 0 || worksheetModel.MergeRegions.Count > 0))
        {
    ...
    private static bool TryResolveRowIndex(XElement rowElement, LoadDiagnostics diagnostics, LoadOptions options, string sheetName, out int rowIndex)
    {
        ...
        foreach (var cellElement in rowElement.Elements(MainNs + "c"))
        {
            ...
            if (TryParseCellReference(cellReference ?? string.Empty, out var address))
            {
        ...
        out CellRecord record)
    {
        ...
        if (TryReadCellValue(cellElement, cellType, valueElement?.Value, isDateStyle, dateSystem, sharedStrings, diagnostics, options, sheetName, resolvedCellReference, out var value, out var kind))
        {
            record.Value = value;
    ...
    private static void LoadMergeRegions(WorksheetModel worksheetModel, XElement worksheetRoot, LoadDiagnostics diagnostics, LoadOptions options, string sheetName)
    {
        foreach (var mergeElement in worksheetRoot.Element(MainNs + "mergeCells")?.Elements(MainNs + "mergeCell") ?? Enumerable.Empty<XElement>())
        {
            ...
            if (!TryParseMergeReference(mergeReference ?? string.Empty, out var region))
            {
        ...
        out CellValueKind kind)
    {
        ...
        if (cellType == "s")
        {
            if (int.TryParse(rawValue, NumberStyles.Integer, CultureInfo.InvariantCulture, out var sharedStringIndex) && sharedStringIndex >= 0 && sharedStringIndex < sharedStrings.Count)
            {
        ...
        if (isDateStyle)
        {
            if (double.TryParse(resolvedRawValue, NumberStyles.Float, CultureInfo.InvariantCulture, out var serial))
            {
        ...
        if (TryParseNumber(resolvedRawValue, out var numberValue))
        {
            value = numberValue;
