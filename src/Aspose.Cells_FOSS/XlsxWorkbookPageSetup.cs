    ...
    internal static class XlsxWorkbookPageSetup
    {
        ...
        internal static Dictionary<int, WorksheetDefinedNamesState> LoadWorksheetDefinedNames(XElement workbookRoot, LoadDiagnostics diagnostics, LoadOptions options)
        {
            ...
            foreach (var definedName in workbookRoot.Element(MainNs + "definedNames")?.Elements(MainNs + "definedName") ?? new XElement[0])
            {
                ...
                if (!states.TryGetValue(localSheetId.Value, out state))
                {
        ...
        private static void LoadHeaderFooter(PageSetupModel pageSetup, XElement headerFooterElement)
        {
            ...
            string leftHeader, centerHeader, rightHeader;
            ParseHeaderFooterText((string)headerFooterElement.Element(MainNs + "oddHeader"), out leftHeader, out centerHeader, out rightHeader);
            ...
            string leftFooter, centerFooter, rightFooter;
            ParseHeaderFooterText((string)headerFooterElement.Element(MainNs + "oddFooter"), out leftFooter, out centerFooter, out rightFooter);
        ...
        private static string NormalizeRowReference(string value)
        {
            ...
            int first, last;
            if (parts.Length != 2
                || !int.TryParse(parts[0], NumberStyles.Integer, CultureInfo.InvariantCulture, out first)
                || !int.TryParse(parts[1], NumberStyles.Integer, CultureInfo.InvariantCulture, out last)
                || first <= 0
                || last < first)
            {
